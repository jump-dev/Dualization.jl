...
The function dualize works in three differente ways. The user can provide:

* A `MathOptInterface.ModelLike`

The function will return a `DualProblem` struct that has the dualized model
in the default `DualizableModel{Float64}` macro and a `PrimalDualMap{Float64}` for 
users to identify the links between primal and dual model.

* A `MathOptInterface.ModelLike` and a `DualProblem{T}` 

The function will fill the provided `DualProblem{T}` with the dualized model. 
This is should only be useful for more advanced use. 
