...
# Generic fallback for `VariableBridgingCost`/`ConstraintBridgingCost` on the
# `DualOptimizer`. The generic `MOI.get(::DualOptimizer, ::AbstractModelAttribute)`
# above goes through `dual_attribute` which has no method for these attributes;
# we override it here so that the standard `(supports ? 0 : Inf)` fallback is
# used when no more specific method below applies.
function MOI.get(
    optimizer::DualOptimizer,
...
# A `VariableBridgingCost{S}` query on the `DualOptimizer` corresponds to the
# cost of adding the dual constraint that is created for the variables in `S`,
# mirroring the structure of `MOI.supports_add_constrained_variables`.
function MOI.get(
    optimizer::DualOptimizer{T},
...
# Adding a scalar constrained variable is equivalent (per the MOI fallback for
# `supports_add_constrained_variable`) to adding a free variable and then a
# `VariableIndex`-in-`S` constraint, so the bridging cost is the sum.
function MOI.get(
    optimizer::DualOptimizer{T},
    ::MOI.VariableBridgingCost{S},
) where {T,S<:MOI.AbstractScalarSet}
    free_cost =
        MOI.get(optimizer, MOI.VariableBridgingCost{MOI.Reals}())::Float64
    if !isfinite(free_cost)
        return free_cost
    end
    constraint_cost = MOI.get(
        optimizer,
        MOI.ConstraintBridgingCost{MOI.VariableIndex,S}(),
    )::Float64
    if !isfinite(constraint_cost)
        return constraint_cost
    end
    return free_cost + constraint_cost
end

# A `ConstraintBridgingCost{F,S}` query on the `DualOptimizer` corresponds to
# the cost of adding the dual constrained variable in `_dual_set_type(S)`,
# mirroring the structure of `MOI.supports_constraint`.
function MOI.get(
    optimizer::DualOptimizer{T},
