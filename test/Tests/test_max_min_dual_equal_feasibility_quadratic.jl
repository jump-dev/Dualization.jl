...
function test_equivalence_max_min(DualMinModel,DualMaxModel)
    for (F,S) in list_of_constraint_types(DualMinModel)
        DualMinModel_eq_con_funs = [MOI.get(
            backend(DualMinModel),
            MOI.ConstraintFunction(),
            ctr_idx,
        ) for ctr_idx in JuMP.index.(all_constraints(DualMinModel,F,S))]
        DualMaxModel_eq_con_funs = [MOI.get(
            backend(DualMaxModel),
            MOI.ConstraintFunction(),
            ctr_idx,
        ) for ctr_idx in JuMP.index.(all_constraints(DualMaxModel,F,S))]
        @test length(DualMinModel_eq_con_funs) == length(DualMaxModel_eq_con_funs)
        for i in eachindex(DualMinModel_eq_con_funs)
            if typeof(DualMinModel_eq_con_funs[i]) != MOI.VariableIndex
                @test MOI.coefficient.(DualMinModel_eq_con_funs[i].terms) == MOI.coefficient.(DualMaxModel_eq_con_funs[i].terms)
                @test MOI.constant.(DualMinModel_eq_con_funs[i]) == MOI.constant.(DualMaxModel_eq_con_funs[i])
            end
        end
        DualMinModel_eq_con_sets = [MOI.get(
            backend(DualMinModel),
            MOI.ConstraintSet(),
            ctr_idx,
        ) for ctr_idx in JuMP.index.(all_constraints(DualMinModel,F,S))]
        DualMaxModel_eq_con_sets = [MOI.get(
            backend(DualMaxModel),
            MOI.ConstraintSet(),
            ctr_idx,
        ) for ctr_idx in JuMP.index.(all_constraints(DualMaxModel,F,S))]
        @test length(DualMinModel_eq_con_sets) == length(DualMaxModel_eq_con_sets)
        for i in eachindex(DualMinModel_eq_con_sets)
            @test MOI.constant.(DualMinModel_eq_con_sets[i]) == MOI.constant.(DualMaxModel_eq_con_sets[i])
        end
    end
end
