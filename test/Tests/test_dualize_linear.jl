@testset "linear problems" begin
    ...
    =#
        ...
        @test MOI._constant(obj) == -1.0
        @test MOI.coefficient.(obj.terms) == [3.0; 3.0; 1.0]
        
        eq_con1_fun = MOI.get(dual_model, MOI.ConstraintFunction(), CI{SAF{Float64}, MOI.EqualTo{Float64}}(4))
        eq_con1_set = MOI.get(dual_model, MOI.ConstraintSet(), CI{SAF{Float64}, MOI.EqualTo{Float64}}(4))
        @test MOI.coefficient.(eq_con1_fun.terms) == [1.0; 1.0; 1.0]
        @test MOI._constant.(eq_con1_fun) == 0.0
        @test MOIU.getconstant(eq_con1_set) == 0.0
        eq_con2_fun = MOI.get(dual_model, MOI.ConstraintFunction(), CI{SAF{Float64}, MOI.EqualTo{Float64}}(5))
        eq_con2_set = MOI.get(dual_model, MOI.ConstraintSet(), CI{SAF{Float64}, MOI.EqualTo{Float64}}(5))
        @test MOI.coefficient.(eq_con2_fun.terms) == [2.0]
        @test MOI._constant.(eq_con2_fun) == 0.0
        @test MOIU.getconstant(eq_con2_set) == -4.0

        primal_con_dual_var = primal_dual_map.primal_con_dual_var
        @test primal_con_dual_var[CI{SAF{Float64}, MOI.LessThan{Float64}}(1)] == [VI(3)]
        @test primal_con_dual_var[CI{SVF, MOI.GreaterThan{Float64}}(2)] == [VI(1)]
        @test primal_con_dual_var[CI{SVF, MOI.GreaterThan{Float64}}(3)] == [VI(2)]

        primal_var_dual_con = primal_dual_map.primal_var_dual_con
        @test primal_var_dual_con[VI(1)] == CI{SAF{Float64}, MOI.EqualTo{Float64}}(4)
        @test primal_var_dual_con[VI(2)] == CI{SAF{Float64}, MOI.EqualTo{Float64}}(5)
    end
    ...
    =#
        ...
        @test MOI._constant.(eq_con2_fun) == 0.0
        @test MOIU.getconstant(eq_con2_set) == -3.0
    ...
    =#
        ...
        primal_con_dual_var = primal_dual_map.primal_con_dual_var
        @test primal_con_dual_var[CI{SVF, MOI.EqualTo{Float64}}(4)] == [VI(1)]
        @test primal_con_dual_var[CI{SVF, MOI.GreaterThan{Float64}}(3)] == [VI(2)]
        @test primal_con_dual_var[CI{SAF{Float64}, MOI.EqualTo{Float64}}(1)] == [VI(3)]
        @test primal_con_dual_var[CI{SAF{Float64}, MOI.EqualTo{Float64}}(2)] == [VI(4)]
    ...
    =#
        ...
        @test obj_type == SAF{Float64}
        obj = MOI.get(dual_model, MOI.ObjectiveFunction{obj_type}())
        @test MOI._constant(obj) == 5.0
        @test MOI.coefficient.(obj.terms) == [3.0; 20.0; 1.0]
        ...
        @test MOI._constant.(eq_con2_fun) == 0.0
        @test MOIU.getconstant(eq_con2_set) == 0.0
        eq_con3_fun = MOI.get(dual_model, MOI.ConstraintFunction(), CI{SAF{Float64}, MOI.EqualTo{Float64}}(6))
        eq_con3_set = MOI.get(dual_model, MOI.ConstraintSet(), CI{SAF{Float64}, MOI.EqualTo{Float64}}(6))
        @test MOI.coefficient.(eq_con3_fun.terms) == [1.0]
        @test MOI._constant.(eq_con3_fun) == 0.0
        @test MOIU.getconstant(eq_con3_set) == 4.0

        primal_con_dual_var = primal_dual_map.primal_con_dual_var
        @test primal_con_dual_var[CI{SVF, MOI.LessThan{Float64}}(3)] == [VI(2)]
        @test primal_con_dual_var[CI{SVF, MOI.LessThan{Float64}}(2)] == [VI(1)]
        @test primal_con_dual_var[CI{SAF{Float64}, MOI.LessThan{Float64}}(1)] == [VI(3)]

        primal_var_dual_con = primal_dual_map.primal_var_dual_con
        @test primal_var_dual_con[VI(1)] == CI{SAF{Float64}, MOI.EqualTo{Float64}}(4)
        @test primal_var_dual_con[VI(2)] == CI{SAF{Float64}, MOI.EqualTo{Float64}}(5)
        @test primal_var_dual_con[VI(3)] == CI{SAF{Float64}, MOI.EqualTo{Float64}}(6)
    end
