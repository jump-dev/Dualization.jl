@testset "rsoc problems" begin
    @testset "rsoc1_test" begin
        ...
        @test MOI.coefficient.(obj.terms) ==
              (Sys.WORD_SIZE == 32 ? [1.0; 0.5] : [0.5; 1.0])
        ...
        eq_con4_fun = MOI.get(
            dual_model,
            MOI.ConstraintFunction(),
            CI{SAF{Float64},MOI.EqualTo{Float64}}(5),
        )
        eq_con4_set = MOI.get(
            dual_model,
            MOI.ConstraintSet(),
        ...
        @test primal_con_dual_var[CI{VVF,MOI.RotatedSecondOrderCone}(1)] ==
              VI.(1:4)
